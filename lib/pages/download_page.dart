...
import '../services/download_file_matcher.dart';
...
class _DownloadPageState extends State<DownloadPage> {
  ...
  bool _singleStreamsLoading = false;
  ...
  Future<void> _parseUrl() async {
    ...
    final urls = BilibiliApi.extractUrls(url);
    ...
    setState(() {
      ...
      _singleStreamsLoading = false;
    });

    // 逐个链接解析，结果合并到批量列表
    final parsed = List<List<BilibiliVideoInfo>?>.filled(urls.length, null);
    final failed = List<bool>.filled(urls.length, false);
    var nextIndex = 0;
    Future<void> worker() async {
      while (true) {
        if (!mounted || !_isParsing || _parseRunId != runId) return;
        ...
        try {
          ...
          if (_isCollectionUrl(resolved)) {
            ...
            // 解析只拿 view 基本信息，不触发下载阶段的 playurl 限速。
            final info = await _api.getVideoInfo(
              resolved,
    ...
    if (!mounted || !_isParsing || _parseRunId != runId) return;
    final parsedItems = <_BatchItem>[];
    for (final infos in parsed) {
      for (final info in infos ?? const <BilibiliVideoInfo>[]) {
        parsedItems.add(
          _BatchItem(
            info: info,
            name: info.bvid,
            exists: _fileExists(info.bvid, _safeName(info.title)),
          ),
        );
      }
    }
    final gotAny = parsedItems.isNotEmpty;
    final failedCount = failed.where((value) => value).length;
    if (gotAny) setState(() => _batchItems = parsedItems);
    if (!gotAny) {
    ...
    if (_batchItems.length == 1 &&
        !_isCollectionUrl(urls.length == 1 ? urls.first : '')) {
      // 单个普通视频：走单曲详情
  ...
  bool _isCollectionUrl(String url) =>
      ...
      (url.contains('list.bilibili.com') &&
          (url.contains('ml') || url.contains('fid'))) ||
  ...
  Future<void> _checkSingleExists(BilibiliVideoInfo info) async {
    ...
    final title = _safeName(info.title);
    final hasAudio = DownloadFileMatcher.hasFile(
      directory: dir,
      bvid: name,
      title: title,
      extension: 'm4a',
    );
    final hasVideo = DownloadFileMatcher.hasFile(
      directory: dir,
      bvid: name,
      title: title,
      extension: 'mp4',
    );
    final hasCover = DownloadFileMatcher.hasFile(
      directory: dir,
      bvid: name,
      title: title,
      extension: 'jpg',
    ...
    _alreadyDownloaded = hasMedia && hasCover;
    _coverMissing = hasMedia && !hasCover;
  ...
  Future<void> _startSingle() async {
    ...
    if (info == null || info.audioUrl == null) {
      _dlog('startSingle: audioUrl null');
      _snack('无下载地址');
      return;
    }
    ...
    final dir = _downloadDir!;
    _dlog(
      'startSingle dir=$dir url=${info.audioUrl!.substring(0, info.audioUrl!.length > 40 ? 40 : info.audioUrl!.length)}... size=${info.audioSize}',
    );
    ...
    if (_downloadAudio) {
      final hasAudio =
          File('$dir/$name.m4a').existsSync() &&
          File('$dir/$name.m4a').lengthSync() > 0;
      if (!hasAudio) {
            ...
            if (now.difference(lastUi).inMilliseconds >= 400) {
              ...
              if (Platform.isAndroid) {
                FlutterForegroundTask.updateService(
                  notificationTitle: 'GoMusic 下载中',
                  notificationText:
                      '${info.title} ${(prog * 100).toStringAsFixed(0)}%',
                );
              }
    ...
    if (_downloadVideo) {
      if (_selectedStream?.baseUrl != null) {
        ...
        try {
          final durl = await _api.resolveVideoDurl(
            info.bvid,
            info.cid,
            _selectedStream!.id,
          );
        ...
        if (info.audioUrl != null &&
          ...
          videoOk = await StreamDownloader.download(
            ...
            cancel: _cancelNotifier,
            expectedSize: _selectedStream!.size > 0
                ? _selectedStream!.size
                : null,
          );
        }
      } else {
    ...
    String sizeText = '';
    try {
      final f = File('$dir/$name.m4a');
      if (f.existsSync())
        sizeText = '${(f.lengthSync() / 1048576).toStringAsFixed(1)} MB';
    } catch (_) {}

    // 最终校验：勾选的音频/视频 + 封面，任一缺失 = 下载失败
    final needAudio = _downloadAudio
        ? (File('$dir/$name.m4a').existsSync() &&
              File('$dir/$name.m4a').lengthSync() > 0)
        : true;
    final needVideo = _downloadVideo
        ? (File('$dir/$name.mp4').existsSync() &&
              File('$dir/$name.mp4').lengthSync() > 0)
        : true;
    final needCover =
        File('$dir/$name.jpg').existsSync() &&
        File('$dir/$name.jpg').lengthSync() > 0;
    ...
    if (!allOk) {
      ...
      _snack(
        '下载失败：文件不完整（${needCover ? "" : "封面"}${needAudio ? "" : "音频"}${needVideo ? "" : "视频"}缺失）',
      );
      return;
    }
    // 完全下载成功后才登记（不受页面切换影响）
    ...
    await _checkSingleExists(info);
    _snack(
      '${audioOk && videoOk ? "下载完成" : (_cancelNotifier?.value == true ? "已取消" : "下载失败")} $sizeText${coverFailed ? " ⚠封面下载失败" : ""}',
    );
  }

  // ==================== 批量下载 ====================

  Future<void> _loadSingleStreams(BilibiliVideoInfo info) async {
    try {
      await _api.loadStreams(info);
    } finally {
      if (!mounted || _singleInfo != info) return;
      setState(() {
        _singleStreamsLoading = false;
        _selectedStream = info.videoStreams.isNotEmpty
            ? info.videoStreams.first
            : null;
      });
    }
  }
  ...
  Future<bool> _downloadItem(_BatchItem item) async {
    ...
    if (full?.audioUrl == null) {
      _dlog(
        '下载: ${item.info.bvid} audioUrl null (cid=${full?.cid}, streams=${full?.videoStreams.length})',
      );
      return false;
    }
    // 封面（失败 = 该项下载失败）
    ...
    if (full!.coverUrl.isNotEmpty) {
      coverOk = await StreamDownloader.download(
        url: full.coverUrl,
        savePath: '${_downloadDir}/${item.name}.jpg',
        onProgress: (_) {},
        cancel: _cancelNotifier,
      );
    } else {
    ...
    if (_downloadAudio) {
      _speedLastBytes = 0;
      _speedLastTime = DateTime.now();
      audioOk = await StreamDownloader.download(
        url: full.audioUrl!,
        savePath: '${_downloadDir}/${item.name}.m4a',
        onProgress: (p) {
          if (mounted) setState(() {});
        },
        onSize: (received, total) {
          if (mounted)
            setState(() {
              _updateItemSpeed(item, received);
    ...
    if (_downloadVideo && full.videoStreams.isNotEmpty) {
      ...
      try {
        ...
        r.stream.drain<void>();
        if (cr != null && cr.contains('/'))
          videoSize = int.tryParse(cr.split('/').last);
        // 更新解析列表显示的大小（下载用 durl，与实际一致）
        if (videoSize != null &&
            videoSize > 0 &&
            item.info.videoStreams.isNotEmpty) {
        ...
        if (videoSize != null &&
            videoSize > 0 &&
            best.size > 0 &&
            videoSize < best.size * 0.7) {
          _snack(
            '当前账号权限不足，视频已降级（${(videoSize / 1048576).toStringAsFixed(0)}MB）',
          );
        }
      } catch (_) {}
      if (full.audioUrl != null &&
          videoUrl == full.audioUrl &&
          audioFile.existsSync()) {
        try {
        ...
        videoOk = await StreamDownloader.download(
          url: videoUrl!,
          savePath: '${_downloadDir}/${item.name}.mp4',
    ...
    final needAudio = _downloadAudio
        ? (File('${_downloadDir}/${item.name}.m4a').existsSync() &&
              File('${_downloadDir}/${item.name}.m4a').lengthSync() > 0)
        : true;
    final needVideo = _downloadVideo
        ? (File('${_downloadDir}/${item.name}.mp4').existsSync() &&
              File('${_downloadDir}/${item.name}.mp4').lengthSync() > 0)
        : true;
    final needCover =
        File('${_downloadDir}/${item.name}.jpg').existsSync() &&
        File('${_downloadDir}/${item.name}.jpg').lengthSync() > 0;
    ...
    final mainFile = _downloadAudio
        ? '${_downloadDir}/${item.name}.m4a'
        : '${_downloadDir}/${item.name}.mp4';
    SongManager.registerSong(
      filePath: mainFile,
      title: full.title,
      uploader: full.author,
      durationSec: full.durationSeconds,
      bvid: full.bvid,
      url: full.url,
        ...
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  if (_isParsing || _singleStreamsLoading) _buildLoading(),
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: IconButton(
                  icon: Icon(
                    Icons.close,
                    size: 18,
                    color: _isParsing ? Colors.red : Colors.grey,
                  ),
                  tooltip: '',
  ...
  Widget _buildSingleResult() {
    ...
    return Column(
      children: [
        ...
        _buildInfoCard(info),
        if (_alreadyDownloaded) _buildAlreadyBadge(),
        if (_coverMissing) _buildCoverMissingBadge(),
        if (_videoMissing) _buildVideoMissingBadge(),
          ...
          Row(
            children: [
              ...
              Switch(
                ...
                onChanged: _isDownloading
                    ? null
                    : (v) => setState(() => _downloadVideo = v),
              ),
            ],
          ),
        ],
        // 下载进度（单独显示，不受条件控制）
        if (_isDownloading) ...[const SizedBox(height: 12)],
      ],
    );
  }
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${info.author} · ${info.durationText}',
                    style: const TextStyle(fontSize: 12, color: Colors.grey),
                  ),
                  const SizedBox(height: 2),
                  Text(
                    '音频 ${info.audioSizeText} | 视频 ${_selectedStream?.sizeText ?? '未知'}',
  ...
  Widget _buildProgressBar() {
    ...
    final totalProg = queueActive && _queueTotal > 0
        ? (_queueDone + curProg) / _queueTotal
        : _downloadProgress;
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Text(
              '已完成 ${_batchItems.where((b) => b.exists || b.status == _BatchStatus.done).length}/${_batchItems.length}',
              style: const TextStyle(fontSize: 12, color: Colors.grey),
            ),
          ],
        ),
  ...
  Widget _buildBatchRow(_BatchItem item) {
    ...
    switch (item.status) {
      case _BatchStatus.done:
        ...
        label = '已下载';
        break;
      case _BatchStatus.downloading:
        icon = Icons.downloading;
        color = Colors.orange;
        label = '下载中';
        break;
      case _BatchStatus.failed:
        icon = Icons.error;
        color = Colors.red;
        label = '失败';
        break;
      case _BatchStatus.waiting:
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          '${item.info.author} · ${item.info.durationText} · 音频${item.info.audioSizeText}',
          style: const TextStyle(fontSize: 11, color: Colors.grey),
        ),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
              ...
              Text(
                item.progress > 0
                    ...
                    : (item.receivedBytes > 0
                          ? '${(item.receivedBytes / 1048576).toStringAsFixed(1)}MB ${item.speed > 0 ? _fmtSpeed(item.speed) : ""}'
                          : '等待中'),
  ...
  Widget? _buildBottomBar() {
    if (_isParsing ||
        _singleStreamsLoading ||
        (!_isDownloading && _singleInfo == null && _batchItems.isEmpty))
      return null;
              ...
              children: [
                if (_downloadingTitle.isNotEmpty)
                  Padding(
                    ...
                    child: Text(
                      '正在下载：$_downloadingTitle${_downloadingSize > 0 ? ' (${(_downloadingSize / 1048576).toStringAsFixed(1)}MB)' : ''}',
                      style: const TextStyle(fontSize: 12),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                    ),
                  ...
                  child: OutlinedButton.icon(
                    onPressed: _cancelling
                        ...
                        : () {
                            ...
                            _dlQueue.removeWhere(
                              (t) =>
                                  t.status == _BatchStatus.waiting && !t.queued,
                            );
                            setState(() => _cancelling = true); // 立即反馈
                          },
                    icon: const Icon(Icons.close, color: Colors.red, size: 16),
                    label: Text(
                      _cancelling ? '正在取消...' : '取消下载',
